...
def get_swft_service() -> SwftComplianceService:
    ...
    try:
        ...
        # Surface missing/invalid configuration as a 503 instead of a generic 500.
        raise HTTPException(status_code=503, detail=f"SWFT compliance backend is not configured: {exc}") from exc
    except Exception as exc:  # pragma: no cover - defensive guardrail
        raise HTTPException(status_code=503, detail=f"Failed to initialise SWFT compliance backend: {exc}") from exc
